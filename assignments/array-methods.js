...
runners.forEach(function (runners) {
  let fullName = [];
  fullName.push("Full Name: " + runners.first_name + " " + runners.last_name)
...
const ticketPriceTotal = runners.reduce(function (tixPrice, item) {
  return tixPrice += item.donation;
}, 0);
...
// Problem 1 Get the company list of all the runners:

const companys = runners.map((company) => {
  return company.company_name;
});
console.log(companys);

// Problem 2 Get the emails of each runner:

const emails = runners.map((emailInfo) => {
  return emailInfo.email;
});
console.log(emails);

// Problem 3 Get the average donation amount:

const average = runners.length;
let donate = runners.reduce((acc, currentValue) => {
